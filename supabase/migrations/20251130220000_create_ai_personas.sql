...
drop policy if exists "Users can view their own persona" on public.ai_personas;
create policy "Users can view their own persona" on public.ai_personas for select using (auth.uid() = user_id);

drop policy if exists "Users can insert their own persona" on public.ai_personas;
create policy "Users can insert their own persona" on public.ai_personas for insert with check (auth.uid() = user_id);

drop policy if exists "Users can update their own persona" on public.ai_personas;
